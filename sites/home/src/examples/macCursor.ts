/* The macOS arrow pointer, rasterised at 2× from NSCursor.arrow (17×23 pt,
   hotspot 4,4) — the same raster metal-fx v2's demo uses. Handed to
   thinking-orbs' gravity so the pointer it draws is pixel-identical to the
   one it hides. Apple artwork: site use only, never shipped in a package.
   Only offered where the platform pointer IS this arrow. */
import type { CursorSprite } from "thinking-orbs";

export const MAC_ARROW: CursorSprite = {
  src: "data:image/png;base64,",
  width: 17,
  height: 23,
  hotX: 4,
  hotY: 4,
};

/** Whether the platform pointer is the arrow above. */
export const isMacPointer = (): boolean =>
  typeof navigator !== "undefined" && /Mac/.test(navigator.platform || "");
